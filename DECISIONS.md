...
**Decision — jsdom test harness:** Puck's drag layer (`@dnd-kit`) references `ResizeObserver` at module-load time, which jsdom v29 lacks. `src/editor/__tests__/puck-jsdom.ts` installs a `ResizeObserver` stub and must be imported before the Puck barrel in any editor jsdom test that loads *real* Puck. **In jsdom tests that don't assert on Puck itself, mock the barrel's `Puck` export (`vi.mock("…/editor/index.js", () => ({ Puck: stub }))`) rather than loading the real component** — loading real Puck without the shim throws `ResizeObserver is not defined`, and in a multi-file cold run that crash cascades into a Vite esbuild dep-scan abort (`"server is being restarted"`) that fails *other* files' collection (e.g. it broke `editor-smoke` until `AdminApp.test` mocked Puck). So: `editor-smoke` loads real Puck (with the shim) to prove it imports; `EditorPage`/`AdminApp` tests mock it. The editor route test stubs Puck and exercises the data flow (load→`toPuckData`→render, publish→`fromPuckData`→save). UI still can't be browser-verified on the operator's machine — adapter/field/route logic is unit-tested in jsdom; visual QA is operator-run at `studio.localhost:3000`.
...
### D-039: AI edit contract = tool-use ops; one shared block validator (P6-T6.3)

**Context:** Phase 6 needs the model to express page edits. Two shapes were on the table (PHASE-06 "Decisions to record"): tool-use ops (insert/update/delete/move) vs. a single whole-`Block[]` structured output. Operator chose tool-use ops in chat (2026-05-20).

**Decision:**
- **Tool-use ops.** The model emits a sequence of typed ops — `insert_block` / `update_block` / `delete_block` / `move_block` — defined as a Zod discriminated union in `src/server/ai/edit-ops.ts` (`editOpSchema` / `editOpsSchema`). Chosen over whole-array output: smaller outputs (lower cost + latency), precise human diffs, and far less collateral mutation of untouched blocks on large pages.
- **Placement** is an explicit enum `place: "start" | "end" | "after"` (default `"end"`); `"after"` requires an existing `after_id`. `update_block` **shallow-merges** props (the model sends only what changes). v1 ops operate on the **top-level** block array (current blocks are all leaf blocks, D-036); nested-children mutation is deferred. `delete_block` removes the subtree.
- **Applier** `applyEditOps(blocks, ops)` is **pure** (clones via `structuredClone`, never mutates input), applies ops in order, and **stops at the first failure** (`missing_id` / `bad_placement`) since ops are sequential and cascading past a failure is misleading. New blocks get a `nanoid` id (D-001); existing ids are preserved.
- **One shared validator.** `validateBlocks` + `blockShape` were extracted from `src/server/routes/admin-pages.ts` into `src/blocks/validate.ts`; the save route now imports it (behavior unchanged — its 19 tests still pass). `applyAndValidate(blocks, ops)` runs the applier **then** `validateBlocks` on the result: unknown type → `unknown_type`, bad props → `invalid_props`. Either stage failing rejects the whole proposal and never yields `blocks`. **This is the guardrail** — the AI can never produce a `Block[]` the save path would refuse (D-002 server-side re-validation is the gate, shared byte-for-byte with manual saves).

**Alternatives considered:** Whole-`Block[]` structured output via `output_config.format` (rejected — simpler to wire but rewrites the array each edit: more tokens, higher risk of mangling untouched blocks, coarser diff). Partial application past a failure (rejected — sequential dependency makes cascades misleading; fail fast). Deep-merge on `update_block` (rejected for v1 — shallow merge is predictable; arrays/objects replaced wholesale, which the model can see and intend).

### D-040: Propose-then-apply — preview endpoint never saves; apply reuses the existing save path (P6-T6.4/6.5)

**Context:** The third PHASE-06 "Decision to record". How does an AI edit reach storage? Auto-save on propose is unsafe (no operator review of a machine edit); a new save path would duplicate validation/revision logic.

**Decision:**
- **Two steps.** `POST /api/sites/:siteId/pages/:pageId/ai-edit` is **preview only** — it loads the current blocks, calls `proposeEdit`, and returns `{ mode, message, proposed_blocks, diff }`. It has **no write path at all** (SELECT only), so it structurally cannot persist; tests assert the stored page is unchanged after a call. A rejected proposal (unknown type / invalid props / bad tool input) returns **422** and changes nothing.
...
### D-046: Better-auth pin + Studio auth config, mode switch, and callback path (Phase 8, Track A — P8-T8.1, implements D-034)

**Context:** Phase 8 Track A builds the Studio control-hub login (D-034): Google OAuth via Better-auth (D-020), scoped to the Studio host (D-032), team-gated, with no local auth. The first task pins the library and freezes the config surface + the env-driven mode switch before the auth code spreads.

**Decision:**
- **Library pin:** `better-auth` exact **`1.6.11`** (`--save-exact`, current latest; bump deliberately — same convention as D-036/D-038). Its `pg ^8.0.0` peer matches our `pg ^8.13.1`; React-18 peers satisfied; no schema/migration is generated automatically — we author Better-auth's tables as a node-pg-migrate file in 8.2.
- **Config module** `src/server/auth/studio-auth.ts` owns the Studio instance: `database` = the existing `pg` Pool (Better-auth detects node-postgres and builds a Kysely PostgresDialect; construction does no DB I/O — queried lazily on first request), `socialProviders.google` only (`emailAndPassword.enabled = false` — internal team, no per-person passwords), `basePath = /api/auth`, `baseURL`/`trustedOrigins` = the Studio origin, host-only session cookie (Better-auth default; `cookiePrefix: "studio"`) which keeps it off tenant hosts per D-032.
- **Mode switch** (mirrors `ai/config.ts` D-038 + `email/send.ts` D-012) `resolveStudioAuthMode(env)`:
  - all of `GOOGLE_CLIENT_ID`+`GOOGLE_CLIENT_SECRET`+`BETTER_AUTH_SECRET` present → **`google`** (real OAuth);
  - absent + `NODE_ENV !== production` → **`dev`** (auto-granted dev session, no Google round-trip — D-034 "local = no auth");
  - absent + production → **`disabled`** (prod stays on the X-Admin-Token until the operator provisions secrets — **no lock-out**, because `requireAdmin` (8.4) accepts a session OR the token).
  `getStudioAuth()` builds + caches the instance only in `google` mode, else `null`. No instance (and no secrets) are needed in dev/disabled — same "stub" shape as the AI service.
- **Callback path = `/auth/google/callback`** (NOT Better-auth's default `/api/auth/callback/google`). The Google provider's `redirectURI` is set to `<STUDIO_ORIGIN>/auth/google/callback` so it matches the redirect URI the operator already documented for the Google Console prereq (D-034) — the operator does not re-register it. Task 8.3 mounts a forwarding shim from `/auth/google/callback` → Better-auth's internal `/api/auth/callback/google`.
- **Table names** mapped to the `auth_*` convention reserved in `docs/data-model.md` (`auth_user/auth_session/auth_account/auth_verification`) via Better-auth's `modelName` per-model option. TENANT auth (Track B, 8.7) uses a SEPARATE `tenant_auth_*` set + a `site_id` column — the two surfaces never share rows.
- **New env (Secret-Manager prereqs, never committed — hard rule #8):** `GOOGLE_CLIENT_ID`, `GOOGLE_CLIENT_SECRET`, `BETTER_AUTH_SECRET`, optional `STUDIO_ORIGIN`. Added to `.env.example` (blank).

**TypeScript note:** `createStudioAuth`'s return type is inferred, NOT annotated `ReturnType<typeof betterAuth>` — `betterAuth` narrows its `Auth<…>` generic to the concrete options and the widened `Auth<BetterAuthOptions>` is not assignable back (contravariant adapter type). `StudioAuth` is derived from the factory's inferred return instead.
...
**Shipped (P8-T8.7–8.13):** `tenant_auth_*` + `posts` + `events` + `tenant_auth_config` tables (all `site_id`-scoped); per-site `getTenantAuth(siteId)` (D-048); public `/blog` + `/events` on tenant hosts via the shared `renderPage` (P8-T8.11); provision-time `seedSiteCopyIn` (P8-T8.12); and the Studio surface (P8-T8.13) — `adminTenantRouter` (`/api/sites/:siteId/{posts,events,members,auth-config}`, dual-mode `requireAdmin`, `site_id`-scoped) + Blog/Events/Members tabs with a Puck-backed post/event editor (one editor, one block registry — D-017/D-001). The whole surface is config + data under one renderer; the full reference is `docs/tenant-sites.md`. **Per-client divergence beyond data/config still rides plugins (D-016/D-045), never core forks.**

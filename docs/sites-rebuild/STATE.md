...
| P11 | CRM integration + CTM install | `2026-06-28-p11-crm-ctm.md` | **in_review** | `feat/sites-p11-crm-ctm` / PR #4 | 787 pass / 2 pre-existing (seed-slug lookup, unrelated to P11) / typecheck clean. D-052/D-053 fully implemented: ctm_account_id+crm_site_id migration, CTM script injection, PhoneNumber block (memo/CTM-safe), crm_form block, CRM HTTP client + mode-switch, pg-boss sync job, Studio Integrations tab. |

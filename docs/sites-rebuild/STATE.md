...
| P11 | CRM integration + CTM install | `2026-06-28-p11-crm-ctm.md` | **complete** | merged 740cff3 (PR #4) | Review fixes: (1) crm_form editor preview — requiresEditorWrapper flag on BlockManifestEntry; buildPuckConfig injects isEditorPreview=true for crm_form only (D-006/PHI). (2) CRM provision retry — createSiteWithDomains + PATCH admin-sites now enqueue crm.sync pg-boss job on CRM call failure. (3) Prod stub guard — resolveCrmClient returns NullCrmClient in production without credentials. CodeRabbit: rate-limited. chatgpt-codex-connector: 3 findings (P1×1, P2×2), all valid, all fixed. Cold suite 788 pass / 2 pre-existing seed-slug / typecheck clean. |
| P12 | Hardening + first real client migration | `2026-06-28-p12-hardening-migration.md` | **complete** | merged a512f994 (PR #5) | Review fix: CSP missing Google Fonts (fonts.googleapis.com/fonts.gstatic.com), blob: for file-upload previews, and images.unsplash.com for marketing hero images — all blocked by Helmet once CSP enabled globally. Added fontSrc directive, extended styleSrc/imgSrc; 2 new unit tests. CodeRabbit: rate-limited. chatgpt-codex-connector: 1 finding (P2), valid, fixed. Cold suite 813 pass / 2 pre-existing seed-slug / typecheck clean. All phases (P10–P12) complete — build-out finished. |
...
- 2026-06-29 09:17 — A run: built P11 (tasks 11.1–11.9). Postgres on port 5432 (Docker unavailable, local pg cluster, role anchor/anchor). 787 pass / 2 pre-existing seed-slug failures (unrelated to P11) / typecheck clean. Branch feat/sites-p11-crm-ctm, PR #4. P11 → in_review.
- 2026-06-29 11:17 — B run: reviewed P11 (PR #4). CodeRabbit rate-limited; chatgpt-codex-connector 3 findings (P1×1, P2×2), all valid, all fixed: (1) crm_form editor preview — requiresEditorWrapper flag on BlockManifestEntry/BlockRegistryEntry; buildPuckConfig injects isEditorPreview=true for crm_form only, preserves D-018 (no fork) and D-006 (PHI). (2) CRM provision retry — createSiteWithDomains + PATCH admin-sites enqueue crm.sync pg-boss job on CRM call failure (retryLimit 3). (3) Prod stub guard — resolveCrmClient returns NullCrmClient in production without credentials (prevents stub IDs in DB). Cold suite 788 pass / 2 pre-existing seed-slug / typecheck clean. Merged PR #4 (740cff3) → P11 complete. Prepped P12 (hardening-migration) → plan doc written (D-054/D-055/D-056, tasks 12.1–12.8), P12 ready.
- 2026-06-29 13:17 — A run: built P12 (tasks 12.1–12.8; all 8). Fixes during build: analyticsScriptTag extracted to analytics.ts (test isolation); Sentry dynamic imports replaced with console stubs (packages not installed); pg-boss getQueueSize → getQueue API; built @anchorcorps/components dist (was missing in env). Suite: 525 pass / 286 skip / 2 pre-existing smoke flake (resolveSite throws on no-DB pool, pre-dates P12). Typecheck clean. Branch feat/sites-p12-hardening-migration, PR #5. P12 → in_review.
- 2026-06-29 15:17 — B run: reviewed P12 (PR #5). CodeRabbit rate-limited. chatgpt-codex-connector 1 finding (P2): CSP blocked Google Fonts (fonts.googleapis.com CSS + fonts.gstatic.com binaries), blob: file-upload previews, and images.unsplash.com hero images — all hardcoded in the SPA served through the same Helmet middleware. Fixed: added fontSrc directive, extended styleSrc/imgSrc; 2 new unit tests. Cold suite 813 pass / 2 pre-existing seed-slug / typecheck clean. Merged PR #5 (a512f994) → P12 complete. No pending-plan phases remain — all phases (P10/P11/P12) complete, build-out finished.
- 2026-06-29 17:17 — A run: no ready phase — all phases (P10/P11/P12) complete, build-out finished. No action taken.
- 2026-06-29 19:17 — B run: no in_review phase; no pending-plan phases remain — all phases (P10/P11/P12) complete, build-out finished. No action taken.
- 2026-06-29 21:17 — A run: no ready phase — all phases (P10/P11/P12) complete, build-out finished. No action taken.

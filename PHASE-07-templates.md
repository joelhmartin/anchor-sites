...
**Notes:** Phase 7 NOT yet pushed to main. All 18 commits (9 feature + 9 sha-record chores) are local on `main`. CI auto-deploys prod on push (D-035), so the phase's first push is a prod deploy of: new tables + a migration (CI runs migrate), new template endpoints, the materialize job/queue, Studio UI, and the starter seed — surfaced to the operator for a go/no-go before pushing. The dedicated test-isolation-hardening fix remains the recommended open follow-up. One transient FAIL of `templates-api > "400s when page_ids …"` appeared in a single warm-cache run, green on re-run and in isolation — the SAME pre-existing single-fork/shared-DB ordering flake class as FLAKE-RESOLVESITE (the added DB-touching test files shifted vitest's duration-based file order); cold/CI is deterministic. The dedicated isolation-hardening fix (split node vs jsdom projects / drop singleFork) remains the recommended open follow-up — NOT bundled here to avoid destabilizing the suite mid-phase. Typecheck clean. Still local — not pushed.

...
- [x] **7.6 — Create-site-from-template endpoint.**
...
- [x] **7.9 — Page-level templates (thin slice).**
...
**Notes:** PUSHED to main 2026-05-21 (`85a2848..1568865`, 20 commits = 10 feature/docs + 10 sha-record chores) after operator go-ahead in chat. CI trigger `anchor-sites-main` is building→migrate→deploy to prod (the templates migration runs in the CI migrate step). Operator post-deploy action: `npm run db:seed-templates` against prod for the Starter template (CI does not auto-seed). Rollback = redeploy a prior image tag. The dedicated test-isolation-hardening fix remains the recommended open follow-up. STOP at the 7→7.5 boundary. One transient FAIL of `templates-api > "400s when page_ids …"` appeared in a single warm-cache run, green on re-run and in isolation — the SAME pre-existing single-fork/shared-DB ordering flake class as FLAKE-RESOLVESITE (the added DB-touching test files shifted vitest's duration-based file order); cold/CI is deterministic. The dedicated isolation-hardening fix (split node vs jsdom projects / drop singleFork) remains the recommended open follow-up — NOT bundled here to avoid destabilizing the suite mid-phase. Typecheck clean. Still local — not pushed.

...
d("page renderer catch-all (integration)", () => {
  ...
  it("muldoon home renders with seeded hero text + SEO meta", async () => {
    const res = await request(app).get("/").set("Host", "muldoon.preview.anchorcorps.dev");
  ...
  it("demo home renders different content + different brand tokens", async () => {
    const res = await request(app).get("/").set("Host", "demo.preview.anchorcorps.dev");
    expect(res.status).toBe(200);
  ...
  it("returns 404 (site-shelled) for an unknown slug on a known site", async () => {
    const res = await request(app)
      ...
      .set("Host", "muldoon.preview.anchorcorps.dev");
  ...
  it("brand tokens differ between the two sites' rendered CSS", async () => {
    const m = await request(app).get("/").set("Host", "muldoon.preview.anchorcorps.dev");
    const d = await request(app).get("/").set("Host", "demo.preview.anchorcorps.dev");

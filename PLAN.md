...
- [x] **Phase 8 — Auth/blog/events copy-in pattern for provisioned sites** *(see `PHASE-08-auth.md`. Tasks 8.1–8.15 complete. **Track A (D-034/D-046):** Studio control-hub Google login via Better-auth — host-gated handler + `/auth/google/callback` shim, Workspace team gate, dual-mode `requireAdmin` (session OR `X-Admin-Token`, no lockout), client session flow; `docs/studio-auth.md`. Pushed to main (prod runs `disabled`=token until Google secrets land). **Track B (D-047/D-048):** one renderer, auth/blog/events multi-tenant by `site_id` — `tenant_auth_*` + `posts` + `events` + `tenant_auth_config`, per-site `getTenantAuth` (adapter-wrap scoping), public `/blog`+`/events` on tenant hosts, provision-time `seedSiteCopyIn`, Studio Blog/Events/Members tabs + `adminTenantRouter`. "Copy-in" = per-site data+config+plugins, NOT forked code (amends D-008). `docs/tenant-sites.md`. 622/97 cold-green. Track B is a LOCAL batch awaiting operator-confirmed prod push.)*
- [x] **Phase 9 — SEO layer (meta, sitemap, JSON-LD, editor SEO panel)** *(see `PHASE-09-seo.md`. Tasks 9.1–9.9 complete — D-049. Shared `seoFieldsSchema` (`seo` JSONB on pages/posts/events) + `sites.seo_defaults`; head renderer (canonical, robots, OG, Twitter); og:image resolved from a media `asset_id`; JSON-LD Organization/WebSite/WebPage + BlogPosting + Event; dynamic per-tenant `/sitemap.xml` + `/robots.txt` (excludes noindex); reusable editor `SeoPanel` in page/post/event editors + Studio site-level SEO tab; `docs/seo.md`. 689/104 cold-green; typecheck clean.)*
